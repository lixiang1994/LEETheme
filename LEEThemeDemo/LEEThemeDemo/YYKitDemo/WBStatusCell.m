...
//    line.backgroundColor = kWBCellLineColor.CGColor;
    ...
    line.lee_theme
    .LeeAddBorderColor(DAY , kWBCellLineColor)
    .LeeAddBorderColor(NIGHT , kWBCellLineColor_Night);
...
//    self.backgroundColor = kWBCellInnerViewColor;
    ...
    self.lee_theme
    .LeeAddBackgroundColor(DAY , kWBCellInnerViewColor)
    .LeeAddBackgroundColor(NIGHT , kWBCellInnerViewColor_Night);
...
//    [_repostButton setBackgroundImage:[UIImage imageWithColor:kWBCellHighlightColor] forState:UIControlStateHighlighted];
    ...
    _repostButton.lee_theme
    .LeeAddButtonBackgroundImage(DAY , [UIImage imageWithColor:kWBCellHighlightColor] , UIControlStateHighlighted)
    .LeeAddButtonBackgroundImage(NIGHT , [UIImage imageWithColor:kWBCellHighlightColor_Night] , UIControlStateHighlighted);
...
//    [_likeButton setBackgroundImage:[UIImage imageWithColor:kWBCellHighlightColor] forState:UIControlStateHighlighted];
    ...
    NSArray *dayColors = @[(id)clear.CGColor,(id)dark.CGColor, (id)clear.CGColor];
    NSArray *nightColors = @[(id)clear.CGColor,(id)light.CGColor, (id)clear.CGColor];
...
//    _line2.colors = colors;
    ...
    _line1.lee_theme
    .LeeAddCustomConfig(DAY , ^(CAGradientLayer *item){
        
        item.colors = dayColors;
    })
    .LeeAddCustomConfig(NIGHT , ^(CAGradientLayer *item){
        
        item.colors = nightColors;
    });
...
//    _bottomLine.backgroundColor = UIColorHex(e8e8e8).CGColor;
    ...
    _bottomLine.lee_theme
    .LeeAddBorderColor(DAY , UIColorHex(e8e8e8))
    .LeeAddBorderColor(NIGHT , UIColorHex(333333));
...
//    _retweetBackgroundView.backgroundColor = kWBCellInnerViewColor;
    ...
    _retweetBackgroundView.lee_theme
    .LeeAddBackgroundColor(DAY , kWBCellInnerViewColor)
    .LeeAddBackgroundColor(NIGHT , kWBCellInnerViewColor_Night);
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (!_retweetBackgroundView.hidden && insideRetweet) {
        [(_retweetBackgroundView) performSelector:@selector(setBackgroundColor:) withObject:isDay ? kWBCellInnerViewHighlightColor : kWBCellInnerViewHighlightColor_Night afterDelay:0.15];
        _touchRetweetView = YES;
    } else {
        [(_contentView) performSelector:@selector(setBackgroundColor:) withObject:isDay ? kWBCellHighlightColor : kWBCellHighlightColor_Night afterDelay:0.15];
...
- (void)touchesRestoreBackgroundColor {
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:_retweetBackgroundView selector:@selector(setBackgroundColor:) object:isDay ? kWBCellInnerViewHighlightColor : kWBCellInnerViewHighlightColor_Night];
    [NSObject cancelPreviousPerformRequestsWithTarget:_contentView selector:@selector(setBackgroundColor:) object:isDay ? kWBCellHighlightColor : kWBCellHighlightColor_Night];
    
    _contentView.backgroundColor = isDay ? [UIColor whiteColor] : UIColorHex(333333);
    _retweetBackgroundView.backgroundColor = isDay ? kWBCellInnerViewColor : kWBCellInnerViewColor_Night;
}
