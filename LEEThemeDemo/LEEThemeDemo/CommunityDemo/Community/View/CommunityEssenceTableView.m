...
@interface CommunityEssenceTableView ()<UITableViewDelegate , UITableViewDataSource>

@property (nonatomic , strong ) NSMutableArray *circleArray; //圈子数据数组

@property (nonatomic , strong ) NSMutableArray *dataArray; //数据源数组

@property (nonatomic , assign ) NSInteger page; //页数

@property (nonatomic , strong ) CommunityHotCircleListView *hotCircleView; //最热圈子视图
...
static NSString * const CommunityEssenceHeaderViewID = @"CommunityEssenceHeaderView";

@implementation CommunityEssenceTableView
...
- (void)initData{
    ...
    [self registerClass:[CommunityEssenceHeaderView class] forHeaderFooterViewReuseIdentifier:CommunityEssenceHeaderViewID];
}
...
- (void)initSubview{
    ...
    _hotCircleView = [[CommunityHotCircleListView alloc] init];
    
    _hotCircleView.frame = CGRectMake(0, 0, self.width, 180.0f);
    
    UIView *headerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.width, self.hotCircleView.height + 10.0f)];
    
    [headerView addSubview:_hotCircleView];
...
- (void)configBlock{
    ...
    _hotCircleView.selectedBlock = ^(CommunityCircleModel *model){
        
        if (weakSelf) {
            
            if (weakSelf.openCircleDetailsBlock) weakSelf.openCircleDetailsBlock(model);
        }
        
    };
    
    _hotCircleView.moreBlock = ^(){
        
        if (weakSelf) {
            
            if (weakSelf.openCircleListBlock) weakSelf.openCircleListBlock();
        }
...
- (void)loadDataWithPage:(NSInteger)page ResultBlock:(void(^)(NSInteger))resultBlock{
    ...
    [self.api loadDataWithPage:page Type:CommunityListTypeEssence ResultBlock:^(RequestResultType requestResultType, id responseObject) {
                ...
                case RequestResultTypeSuccess:
                {
                    ...
                    if ([responseObject[@"error"] integerValue] == 0) {
                        ...
                        [strongSelf.api analyticalDataWithData:responseObject ResultBlock:^(NSDictionary *result) {
                            ...
                            NSArray *circleArray = result[@"circleArray"];
                            
                            if (circleArray) {
                                
                                [strongSelf.circleArray removeAllObjects];
                                
                                [strongSelf.circleArray addObjectsFromArray:circleArray];
                                
                                [strongSelf.hotCircleView configDataArray:strongSelf.circleArray];
                            }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    CommunityEssenceHeaderView *view = [tableView dequeueReusableHeaderFooterViewWithIdentifier:CommunityEssenceHeaderViewID];
    
    [view configTitle:@"精华帖子"];
