...
- (void)loadAPIGETRequestNoCacheWithURL:(NSString *)url resultBlock:(APIRequestResultBlock)resultBlock{
    ...
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:url] cachePolicy:NSURLRequestReloadIgnoringLocalCacheData timeoutInterval:60.0f];
    ...
    [[session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (error) {
                ...
                if (resultBlock) resultBlock(RequestResultTypeError , error);
